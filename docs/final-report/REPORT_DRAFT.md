...
本课程设计实现住房公积金管理系统中的筹集子系统。系统围绕缴存单位和缴存职工的账户管理展开，主要完成系统参数维护、单位开户、个人开户、资料修改和信息查询等业务。正文中如出现“归集”，仅用于说明住房公积金缴存筹集业务的通用含义。项目采用传统 Maven SSM Web 技术路线，后端使用 Spring、Spring MVC、MyBatis，前端使用 JSP、HTML、CSS 和 JavaScript，数据库使用 MySQL，最终以 WAR 包部署到 Tomcat。
...
用例图可使用 `DIAGRAMS.md` 中的“系统用例图”。主要参与者为综合柜员和系统管理员/运维人员，主要用例包括系统参数维护、单位开户、个人开户、个人 Excel 批量开户、单位资料修改、个人资料修改、单位信息查询和个人信息查询。
...
4. 个人 Excel 批量开户：在个人开户模块的批量区域填写单位公积金账号，上传 Excel 文件，按“序号、个人姓名、证件类型、证件号码、缴存基数”5 列批量开户；任一数据行失败时整体不提交，并显示逐行失败原因。
...
7. 单位信息查询：支持按单位公积金账号精确查询和按单位名称模糊查询，显示单位账户、缴存比例、月汇缴金额、人数等核心信息。
8. 个人信息查询：支持按个人公积金账号和证件号码查询，显示单位信息、姓名、证件号码、缴存基数、缴存比例、月汇缴金额、余额和状态等信息。
...
3. 金额、缴存基数、月汇缴金额等字段使用 `BigDecimal` 和 `DECIMAL(16,2)`，避免浮点数误差。
...
- `TB003`：个人基本资料表，保存个人账号、所属单位、姓名、证件信息、缴存基数、缴存比例、月汇缴金额、余额和账户状态。
...
Excel 批量开户采用“全部成功才提交”的事务策略。系统先读取页面提交的单位公积金账号，再解析 5 列 Excel 并逐行复用个人开户校验逻辑，若存在任一失败行，则本批次不新增任何个人账户，不更新单位汇总，也不递增个人账号序号。

个人资料强制变更中，若 A 账户要改为被 B 账户占用的证件号码，系统使用 `TB001.PERACCNUM` 生成新个人账号 C，C 复制 B 原信息并使用 `9` + B 原证件号码后 17 位作为错误证件号保存；A 更新为正确证件号码。该过程在事务内完成。
...
个人开户入口为 `/persons/open`，对应 `PersonController#open` 和 `PersonServiceImpl#openPerson`。系统先校验单位账号必须存在且为正常状态，再校验姓名、证件类型、证件号码和缴存基数；缴存比例从单位信息表读取并校验。新开户写入 `TB003`，并更新 `TB002` 的人数、基数、单位月汇缴金额和个人月汇缴金额。

### 3.5 Excel 批量开户实现

Excel 批量入口为 `/persons/open/import`，对应 `PersonController#importPersons` 和 `PersonServiceImpl#importPersons`。批量区域提交单位公积金账号，系统使用 Apache POI 解析 `.xls/.xlsx` 的 5 列数据，跳过空行，检查 Excel 内重复证件号、单位状态、证件号占用和缴存基数格式。导入执行时复用 `validateOpenCandidate` 和 `openPerson`，保证批量导入与手工开户校验一致。
...
实机验证覆盖首页、系统参数维护、单位开户、个人手工开户、Excel 批量开户、单位资料修改、个人资料修改、单位信息查询和个人信息查询。Excel 批量开户验证了 5 列模板、失败批次整体回滚和成功批次写入 2 条数据。个人资料强制变更验证了冲突信息完整回显、目标账户获得正确证件号、新建错误账户保存原占用账户信息且错误证件号为 `9` 开头，以及 `TB001.PERACCNUM` 正确递增。

详细测试用例见 `TEST_CASES.md`，截图插入位置见 `SCREENSHOT_INSERTION_GUIDE.md`。

## 第5章 课程设计总结

本次课程设计完成了住房公积金管理系统——筹集子系统的核心功能，实现了从数据库设计、后端业务处理、前端 JSP 页面、MyBatis SQL 到 Tomcat 部署的完整流程。开发过程中重点解决了账号序号生成、事务一致性、Excel 批量导入、个人资料强制变更、数据库字段与指导书对齐、本地数据库密码隔离等问题。

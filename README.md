...
2026-06-18 符合性审查将 `tb003.IDTYPE` 默认值调整为 `01身份证`，应用层开户和修改会显式写入该值。已按当前脚本新建的测试库不需要重新导入；已有库如需保持默认值一致，可手工调整该字段默认值。
...
- 2026-06-18：完成指导书 2.2.1-2.2.7 功能符合性审查和小修复后，使用 `D:\dev\apache-maven-3.9.16\bin\mvn.cmd clean package`。
- 结果：构建成功，62 个测试通过，生成 `target/housingfund-collection.war`。
...
- 个人资料修改模块：按个人账号查询并反显个人资料和所属单位，只允许修改姓名、证件类型、证件号码；身份证号被其他账户占用时支持占用信息回显和强制变更
...
- 不修改联系电话和联系地址；数据库字段保留但不作为个人资料修改交易要素。
...
2. 访问 `/persons/edit`，输入个人账号 `000000000001` 查询，确认页面反显个人账号、单位账号、单位名称、姓名、证件类型、身份证号。
3. 将姓名改为 `李四修改`，或将证件号码改为未被占用的合法身份证号后提交。
...
9. 输入身份证号 `123456` 或证件类型非 `01身份证`，确认页面或后端提示对应校验错误。
...
- 填写个人开户表单并提交。
- 先输入单位账号查询正常单位，反显单位名称、单位比例和个人比例。
...
2. 访问 `/persons/open`，输入该单位账号，例如 `000000000001`，点击查询单位，确认反显单位名称、单位比例和个人比例。
3. 输入个人数据：姓名 `李四`，证件类型 `01身份证`，身份证号 `11010519491231002X`，缴存基数 `5000.00`。
